...
class PortalSmokeTests(SimpleTestCase):
    def test_homepage_renders(self):
        ...
        self.assertContains(response, "The spare is untouched by default")
        ...
        self.assertContains(response, "data-flow-variant-trigger=", count=3)
        self.assertContains(response, "data-flow-variant=\"spare\"", count=1)
        self.assertContains(response, "Spare Panel<br>Replacement", count=1)
        self.assertContains(response, "Panel Upgrade", count=1)
        self.assertContains(response, "General Flow", count=1)
        self.assertContains(response, "Example update context")
        self.assertContains(response, "Example spare context")
    ...
    def test_raft_trial_page_renders_live_panellock_flow(self):
        ...
        self.assertContains(response, "General Flow", count=1)
        self.assertContains(response, "Example update context")
        self.assertContains(response, "data-flow-payload hidden", count=1)
        self.assertContains(response, "Ignition 8.3.6 &rarr; 8.3.8")
        self.assertContains(response, "Example spare context")
        self.assertContains(response, "Existing Panel PC project")
        self.assertContains(response, "Protected backup / hash verified")
        self.assertContains(response, "/static/portal/raft.css")
        self.assertContains(response, "/static/portal/raft.js")
